...
from brtp.misc.utils import zip_random
...
def test_zip_random_invariants(seed: int | None):
    ...
    # --- assert ------------------------------------------
    assert {v for v, _ in result} == set(i1)
    assert [v for v, _ in result] != i1
    assert {v for _, v in result} == set(i2)
    assert [v for _, v in result] != i2
